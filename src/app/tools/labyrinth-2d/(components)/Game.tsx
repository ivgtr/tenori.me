...
export default function Game() {
	const { maze, playerPos, gameStarted, startGame, handleMove, elapsedTime } =
		useMazeGame();
	...
	// ゴールエフェクトのアニメーション
	const animateGoalEffect = useCallback(() => {
	...
	}, [playerPos, maze, animateGoalEffect]);
	...
	return (
      ...
      `}
		...
		>
			...
			/>
			<div
				className={`mt-4 text-center w-full ${isGoalEffect ? "animate-pulse" : ""}`}
			>
				{!gameStarted ? (
					<button
						onClick={startGame}
					...
					<>
						...
						<div
							className={`grid grid-cols-3 gap-2 max-w-[200px] mx-auto ${isGoalEffect ? "animate-bounce" : ""}`}
						>
